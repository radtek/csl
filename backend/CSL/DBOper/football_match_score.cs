...
namespace DBOper
{
    public class football_match_score
    {
        public static string Insert(string football_match_id, string sys_user_id, string match_score_type_id, string amount, string USER, string TOKEN)
        {
            ...
            ds = DBHelper.Select("football_match_score", "id,match_score_type_id,amount", string.Empty, fdict, "and");
            ...
            using (MySqlConnection dbConnection = new MySqlConnection(DBHelper.strConnection))
            {
                ...
                using (MySqlTransaction trans = dbConnection.BeginTransaction())
                {
                    ...
                    if (ds.Tables[0].Rows.Count == 0)
                    {
                        ...
                        dict.Add("match_score_type_id", match_score_type_id);
                        dict.Add("amount", amount);
                        dict.Add("vote_datetime", DateTime.Now.ToString());
                        DBHelper.Insert("football_match_score", dict, dbConnection, trans);
                    }
                    else
                    {
                        if (amount.Equals(ds.Tables[0].Rows[0]["amount"].ToString()) && match_score_type_id.Equals(ds.Tables[0].Rows[0]["match_score_type_id"].ToString())) return string.Empty;
                        ...
                        DBHelper.Update("football_match_score", dict, fdict, "and", dbConnection, trans);
                    ...
                    sys_user_balance_change.Insert(sys_user_id, "7", "-" + amount, round_name + " " + home_football_team_name + "VS" + away_football_team_name + " " + dict_match_score_type.GetName(match_score_type_id), "football_match", football_match_id, dbConnection, trans);
        ...
        public static DataSet GetAll(string football_match_id, string sys_user_id, string match_score_type_id, string pageSize, string pageIndex, string USER, string TOKEN)
        {
            ...
            if (match_score_type_id.Length > 0) fdict.Add("match_score_type_id", match_score_type_id);
            return DBHelper.SelectPager("football_match_score_view", "*", string.Empty, fdict, "and", pageSize, pageIndex);
        }
        public static DataSet GetOne(string id, string USER, string TOKEN)
        {
            ...
            IDictionary<string, string> fdict = new Dictionary<string, string>();
            fdict.Add("id", id);
            return DBHelper.Select("football_match_score_view", "*", string.Empty, fdict, "and");
        }
    }
}
